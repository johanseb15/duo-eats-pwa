...
const OrderColumn = ({ title, orders, onSelectStatus, isUpdating }: OrderColumnProps) => (
    <div className="flex flex-col gap-4">
      <h3 className="text-xl font-bold text-center">{title} ({orders.length})</h3>
      <div className="space-y-4 rounded-xl bg-muted/30 p-4 min-h-[300px]">
        {orders.map(order => (
      ...
      {orders.length === 0 ? (
        ...
        <ScrollArea className="w-full whitespace-nowrap rounded-lg">
            <div className="flex space-x-6 p-1">
                <OrderColumn title="Pendiente" orders={ordersByStatus['Pendiente']} onSelectStatus={onStatusSelect} isUpdating={isUpdating} onStatusChange={handleStatusChange} />
