    ...
    <div className="flex flex-col min-h-screen bg-background pb-28">
      ...
      <main className="flex-grow container mx-auto px-4 py-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <Button onClick={loadOrders} disabled={loading}>Refrescar</Button>
        </div>
                    ...
                    {order.items.map((item, index) => (
                      <li key={`${item.id}-${index}`} className="flex justify-between text-sm p-2 rounded-lg bg-muted/30">
                        <div>
                           ...
                           {item.selectedOptions && <p className='text-xs text-muted-foreground italic'>({Object.values(item.selectedOptions).join(', ')})</p>}
                        </div>
